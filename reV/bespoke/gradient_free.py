...
class GeneticAlgorithm():
    ...
    def initialize_limited(self):
        ...
        self.parent_population = np.zeros((self.population_size,self.nbits),\
            dtype=int)
    ...
    def optimize_ga(self,initialize="random",crossover="random",print_progress=True,save_progress=False,start_individual=np.array([])):
        ...
        for i in range(self.nvars):
            if self.variable_type[i] == "float":
                ...
                self.discretized_variables["float_var%s"%float_ind] = np.linspace(self.bounds[i][0],\
                    self.bounds[i][1],ndiscretizations)
                float_ind += 1
        ...
        while converged==False and ngens < self.max_generation and run_time < self.max_time:
            # crossover
            if crossover=="random":
                self.crossover()
...
class RemovalAlgorithm():
    """wind turbine removal algorithm"""

    def __init__(self):

        # inputs
        self.objective_function = None # of the form objective_function(turbine_x,turbine_y,args)
        self.x_locations = np.array([]) # turbine x locations, assumed to all meet boundary and spacing constraints
        self.y_locations = np.array([]) # turbine y locations, assumed to all meet boundary and spacing constraints
        self.args = () # additional arguments needed by the objective function (wind resource, floris model, etc)

        # outputs
        self.solution_history = np.array([])


    def optimize_remove(self,n_remove=False):

        converged = False
        nlocs = len(self.x_locations)
        nturbs = len(self.x_locations)
        self.solution_history = np.zeros(len(self.x_locations))
        self.solution_history[0] = self.objective_function(self.x_locations,self.y_locations,self.args)
        if n_remove == False:
            n_remove = nturbs
        while converged == False:
            best_answer = self.solution_history[nlocs-nturbs]
            s = time.time()
            remove_order = np.arange(np.min([n_remove,nturbs]))
            np.random.shuffle(remove_order)
            for k in range(len(remove_order)):
                temp_x = np.delete(self.x_locations,remove_order[k])
                temp_y = np.delete(self.y_locations,remove_order[k])
                temp_performance = self.objective_function(temp_x,temp_y,self.args)
                if temp_performance < best_answer:
                    best_answer = temp_performance
                    best_index = remove_order[k]

            if best_answer < self.solution_history[nlocs-nturbs]:
                self.x_locations = np.delete(self.x_locations,best_index)
                self.y_locations = np.delete(self.y_locations,best_index)
                nturbs = len(self.x_locations)
                self.solution_history[nlocs-nturbs] = best_answer

            else:
                self.solution_history = [x for x in self.solution_history if x != 0.0]
                converged = True
...
class AdditionAlgorithm():
    ...
    def optimize_add(self):
        ...
        if nturbs == 0:
            min_arg = np.argmin(self.x_locations+self.y_locations)
            self.turbine_x = np.array([self.x_locations[min_arg]])
            self.turbine_y = np.array([self.y_locations[min_arg]])

            self.x_locations = np.delete(self.x_locations,min_arg)
            self.y_locations = np.delete(self.y_locations,min_arg)

        converged = False
        nturbs = len(self.turbine_x)
        self.solution_history = np.zeros(len(self.x_locations))
        self.solution_history[0] = self.objective_function(self.turbine_x,self.turbine_y,self.args)
        index = 0
        counter = 0

        overall_best = self.solution_history[0]

        while converged == False:
            # best_answer = self.solution_history[index]
            best_answer = 1E20
            temp_x = np.zeros(nturbs+1)
            temp_y = np.zeros(nturbs+1)
            temp_x[0:nturbs] = self.turbine_x[:]
            temp_y[0:nturbs] = self.turbine_y[:]
            nlocs = len(self.x_locations)
            for k in range(nlocs):
                temp_x[-1] = self.x_locations[k]
                temp_y[-1] = self.y_locations[k]
                temp_performance = self.objective_function(temp_x,temp_y,self.args)
                if temp_performance < best_answer:
                    best_answer = temp_performance
                    best_index = k

            if best_answer < overall_best:
                self.turbine_x = np.append(self.turbine_x,self.x_locations[best_index])
                self.turbine_y = np.append(self.turbine_y,self.y_locations[best_index])

                self.x_locations = np.delete(self.x_locations,best_index)
                self.y_locations = np.delete(self.y_locations,best_index)

                nturbs = len(self.turbine_x)
                self.solution_history[index+1] = best_answer
                overall_best = best_answer
                # print(index)
                # print(self.solution_history[0:10])
                index += 1
                counter = 0
                best_x = np.copy(self.turbine_x)
                best_y = np.copy(self.turbine_y)

            elif counter < self.max_counter:
                self.turbine_x = np.append(self.turbine_x,self.x_locations[best_index])
                self.turbine_y = np.append(self.turbine_y,self.y_locations[best_index])
                ...
                self.solution_history = [x for x in self.solution_history if x != 0.0]
                self.turbine_x = np.copy(best_x)
                self.turbine_y = np.copy(best_y)
                converged = True
